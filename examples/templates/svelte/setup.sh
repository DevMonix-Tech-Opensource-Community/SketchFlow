...
APP_NAME="sketchflow-svelte-demo"
...
pnpm create vite "$APP_NAME" --template svelte-ts >/dev/null 2>&1

cd "$APP_NAME"

mkdir -p src/lib

component_file=$(find "$OUTPUT_DIR" -maxdepth 1 -type f -name '*.svelte' | head -n1)

if [[ -z "$component_file" ]]; then
  echo "No Svelte component file generated by Sketchflow CLI" >&2
  exit 1
fi

component_basename=$(basename "$component_file")
component_name="${component_basename%.svelte}"

cp "$component_file" "src/lib/$component_basename"

cat <<EOF > src/routes/+page.svelte
<script lang="ts">
  import ${component_name} from "$lib/${component_basename}";
</script>
...
echo "Svelte starter created in $APP_NAME. Run 'cd $APP_NAME && pnpm dev' to start the dev server."
