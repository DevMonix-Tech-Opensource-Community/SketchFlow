...
pnpm create vite "$APP_NAME" --template vue-ts >/dev/null 2>&1

cd "$APP_NAME"

mkdir -p src/components

component_file=$(find "$OUTPUT_DIR" -maxdepth 1 -type f -name '*.vue' | head -n1)

if [[ -z "$component_file" ]]; then
  echo "No Vue component file generated by Sketchflow CLI" >&2
  exit 1
fi

component_basename=$(basename "$component_file")
component_name="${component_basename%.vue}"
...
echo "Vue starter created in $APP_NAME. Run 'cd $APP_NAME && pnpm dev' to start the dev server."
