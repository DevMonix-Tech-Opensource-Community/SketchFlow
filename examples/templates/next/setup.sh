...
pnpm dlx create-next-app@latest "$APP_NAME" --typescript --app --use-pnpm --yes >/dev/null 2>&1

cd "$APP_NAME"

mkdir -p app/components

component_file=$(find "$OUTPUT_DIR/app/components" -maxdepth 1 -type f -name '*.tsx' | head -n1)
...
cp "$component_file" "app/components/$component_basename"

if [[ ! -f "$OUTPUT_DIR/app/page.tsx" ]]; then
  echo "No page.tsx generated by Sketchflow CLI" >&2
  exit 1
fi

cp "$OUTPUT_DIR/app/page.tsx" app/page.tsx

pnpm install >/dev/null 2>&1

echo "Next.js starter created in $APP_NAME. Run 'cd $APP_NAME && pnpm dev' to start the dev server."
