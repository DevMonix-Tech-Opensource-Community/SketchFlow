...
APP_NAME="sketchflow-solid-demo"
...
pnpm create vite "$APP_NAME" --template solid-ts >/dev/null 2>&1
...
if [[ -z "$component_file" ]]; then
  echo "No Solid component file generated by Sketchflow CLI" >&2
  exit 1
fi
...
echo "Solid starter created in $APP_NAME. Run 'cd $APP_NAME && pnpm dev' to start the dev server."
