...
import { preactAdapter } from "./index.js";
...
describe("preactAdapter", () => {
  it("produces a Preact component file", () => {
    const result = preactAdapter.generate(context);
    expect(result.files).toHaveLength(1);
    const file = result.files[0];
    expect(file.path).toBe("GeneratedScreen.tsx");
    expect(file.contents).toContain("FunctionComponent");
    expect(file.contents).toContain("Hello Preact");
  });

  it("reports metadata", () => {
    const meta = preactAdapter.introspect(context);
