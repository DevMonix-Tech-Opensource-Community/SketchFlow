...
const generatePreactComponent = (context: AdapterContext, options: PreactAdapterOptions): AdapterResult => {
  const componentName = options.componentName ?? DEFAULT_COMPONENT_NAME;
  const body = renderNodes(context.layout, 6);

  const contents = `import { h } from "preact";
...
export const preactAdapter: FrameworkAdapter = {
  ...
  generate: (context: AdapterContext) => {
    const options = (context.options ?? {}) as PreactAdapterOptions;
    return generatePreactComponent(context, options);
  }
};

export const registerPreactAdapter = () => {
  registerFrameworkAdapter(preactAdapter);
